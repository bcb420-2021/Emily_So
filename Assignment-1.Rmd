...
```{r images,fig.cap="Fig 1: The distribution of the original data"}
#distribution 
...
```{r boxplot, warning=FALSE, fig.cap=" Fig 2: The boxplot of the original data"}
#boxplot 
originalData2Plot <- log2(expression[2:20])
boxplot(originalData2Plot,las=2)
```

```{r bcv, fig.cap="Fig 3: The MDS plot showing the clustering of expression values among samples"}
#BCV plot 
...
```
```{r,fig.cap="Fig 4: The BCV plot to measure the variance within the original data"}
#BCV
...
```{r normalizedimages,fig.cap="Fig 5: The distribution of the data after normalization and filtering"}
#distribution 
...
```{r normboxplot, warning=FALSE,fig.cap="Fig 6: The boxplot of the normalized filtered data"}
#boxplot 
...
```{r normBCV,fig.cap="Fig 7: The clustering of the normalized filtered data"}
...
plotMDS(list,labels = rownames(samples),
        col = c("red","blue")[factor(samples$group)])
```
```{r, fig.cap="Fig 8: The BCV plot demonstrating the variance in the data after filtering and normalization"}
#BCV
ModelDesign <- model.matrix(~samples$group)
d <- estimateDisp(list,ModelDesign)
plotBCV(d,col.tagwise = "black",col.common = "red")
